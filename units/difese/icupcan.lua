-- se modifichi il nome della unità ricordati di modificare anche il nome nello script "explosion_spawn_defs.lua"
return {
	icupcan = {
		acceleration = 0,
		antiweapons = 1,
		bmcode = 0,
		brakerate = 0,
		buildangle = 32700,
		buildcostenergy = 8000,
		buildcostmetal = 960,
		builder = false,
		buildtime = 45000,
		buildpic = "icupcan.png",
		canattack = true,
		canstop = 1,
		category = "ALL NOTMOBILE NOTSUB NOTSUBNOTSHIP NOTVTOL WEAPON SURFACE",
		corpse = "DEAD",
		defaultmissiontype = "GUARD_NOMOVE",
		description = "Patroller Cannon",
		energymake = 0,
		energystorage = 0,
		energyuse = 0,
		explodeas = "ATOMIC_BLAST",
		firestandorders = 0,
		footprintx = 4,
		footprintz = 4,
		hightrajectory = 1,
		icontype = "building",
		idleautoheal = 5,
		idletime = 1800,
		mass = 900,
		maxdamage = 4200,
		maxslope = 12,
		maxvelocity = 0,
		maxwaterdepth = 0,
		metalstorage = 0,
		name = "Patroller Cannon",
		nochasecategory = "ALL",
		objectname = "icupcan.s3o",
		seismicsignature = 0,
		selfdestructas = "ATOMIC_BLAST",
		side = "ARM",
		sightdistance = 273,
		standingfireorder = 0,
		turninplaceanglelimit = 140,
		turninplacespeedlimit = 0,
		turnrate = 0,
		unitname = "armpcan",
		workertime = 0,
		yardmap = "oooooooooooooooo",
		customparams = {
			canareaattack = 1,
		},
		featuredefs = {
			dead = {
				blocking = true,
				category = "corpses",
				collisionvolumeoffsets = "3.15924835205 -0.787798413086 6.6563873291",
				collisionvolumescales = "81.6838531494 113.772003174 72.5632324219",
				collisionvolumetype = "Box",
				damage = 2520,
				description = "PeeWee Cannon Wreckage",
				energy = 0,
				featuredead = "HEAP",
				featurereclamate = "SMUDGE01",
				footprintx = 3,
				footprintz = 3,
				height = 40,
				hitdensity = 100,
				metal = 720,
				object = "ARMPCAN_DEAD",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
			heap = {
				blocking = false,
				category = "heaps",
				damage = 1512,
				description = "PeeWee Cannon Heap",
				energy = 0,
				featurereclamate = "SMUDGE01",
				footprintx = 3,
				footprintz = 3,
				height = 4,
				hitdensity = 100,
				metal = 576,
				object = "3X3E",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
	  	                collisionvolumescales = "55.0 4.0 6.0",
		                collisionvolumetype = "cylY",
			},
		},
		sfxtypes = {
			explosiongenerators = {
				[1] = "custom:berthaflare",
			},
		},
		sounds = {
			canceldestruct = "cancel2",
			underattack = "warning1",
			cant = {
				[1] = "cantdo4",
			},
			count = {
				[1] = "count6",
				[2] = "count5",
				[3] = "count4",
				[4] = "count3",
				[5] = "count2",
				[6] = "count1",
			},
			ok = {
				[1] = "servlrg3",
			},
			select = {
				[1] = "servlrg3",
			},
		},
		weapondefs = {
			icupatroller_cannon = {
				accuracy = 500,
				areaofeffect = 20,
				avoidfriendly = false,
				collidefriendly = false,
				craterboost = 0.15,
				cratermult = 0.15,
				energypershot = 890,
			explosiongenerator = "custom:POPUPFLAREFAST",
				gravityaffected = "false",
				impulseboost = 0.1,
				impulsefactor = 0.1,
				metalpershot = 45,
				model = "icupatroller_bomb.s3o",
				name = "PeeWee Cannon",
				noselfdamage = true,
				range = 1650,
				reloadtime = 7,
				soundhitdry = "xplonuk1",
				soundstart = "xplonuk4",
				sprayangle = 1200,
				turret = true,
				weapontype = "Cannon",
				weaponvelocity = 400,
				wobble = 1800,
				damage = {
					default = 0,
				},
			},
		},
		weapons = {
			[1] = {
				def = "icupatroller_cannon",
               		        onlytargetcategory = "SURFACE",
			},
		},
	},
}
