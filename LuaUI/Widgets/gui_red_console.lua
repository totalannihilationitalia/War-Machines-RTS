...
local Config = {
	console = {
		px = 10,py = 25, 	--posizione iniziale
		sx = 516, 			--dimensione x del background
		ancora_x = 10,  -- 10 pixel da sinistra
		ancora_y = 450, -- 450 pixel dal fondo (regola l'altezza per non coprire i menu)		
		fontsize = 10,
	...
	if ((LastAutoResizeX ~= vsx) or (LastAutoResizeY ~= vsy)) then
		...
		for i=1,#objects do
			...
			if not isSlave[o] then
				...
				-- ANCORAGGIO FISSO (Pixel Perfect)
				if (o.ancora_x) then 
...
local function updateconsole(g,cfg)
	...
	if (count == 0) then
		...
		g.vars._empty = true
		g.background.sy = math.floor(cfg.minlines*g.lines.fontsize + (g.lines.px-g.background.px)*2 + 0.5) --g.background.sy = cfg.minlines*g.lines.fontsize + (g.lines.px-g.background.px)*2
	else
		g.background.active = nil --activate
		g.lines.active = nil --activate
		g.vars._empty = nil
		g.background.sy = math.floor(count*g.lines.fontsize + (g.lines.px-g.background.px)*2 + 0.5) --g.background.sy = count*g.lines.fontsize + (g.lines.px-g.background.px)*2
	end
