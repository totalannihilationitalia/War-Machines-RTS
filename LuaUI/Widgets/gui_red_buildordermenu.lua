...
local buttonTexture		= LUAUI_DIRNAME.."images/menu/buildmenu/button.png"
local showNoobButtons   = tonumber(Spring.GetConfigInt("XTA_ShowNoobButtons",1) or 1) == 1
local iconsdimensions	= 50
local margine			= 5
local x_Nicon_build		= 5
local y_Nicon_build		= 3
local x_Nicon_order		= 4
local y_Nicon_order		= 3
local x_orderMenu		= 272
local x_buildMenu		= 320	-- viene poi calcolato successivamente, e verrà dopo l'order menu
local intButtonsX		= 1
local intButtonsY		= 1
...
local Config = {
	buildmenu = { -- menu delle costruzioni (unità)
		...
		ancora_x = x_orderMenu + 2* margine + x_Nicon_order * iconsdimensions + intButtonsX * x_Nicon_order+5,			-- posizione globale da sinistra
		ancora_y = 165,					-- posizione globale dal fondo
		isx = iconsdimensions,
		isy = iconsdimensions, 
		ix = x_Nicon_build,				-- n° icone per riga
		iy = y_Nicon_build, 			-- n° icone per colonna
		
		roundedPercentage = 0.2,		-- 0.25 == iconsize / 4 == cornersize -- era 0.2
		
		iconscale = 1,
		iconhoverscale = 1,
		ispreadx=intButtonsX,			--spazio tra le icone x
		ispready=intButtonsY, 			--spazio tra le icone y
		
		margin = margine, --distance from background border -- offfset del riquadro esterno rispetto i bottoni del menù
...
--		color2 = {0,0,0,1}, -- {0,0,0,0.5} for border effect -- colore riquadro interno (che ho rimosso, vedi righe successive)
		...
		cbackground = {0,0,0,0}, -- {0.03,0.18,0.3,0.5} lo nascondo per lasciar spazio all'immagine -- molix
		cborder = {0,0,0,0},  -- bordo del riquadro esterno era azzurro {0,0.67,0.99,1} lo nascondo per lasciar spazio all'immagine -- molix
		background_texture = ":n:"..LUAUI_DIRNAME.."images/menu/buildmenu/build_bkgnd.png", -- ############## todo ################ impostare small, medium, big ecc le varie taglie dimensionali. molix
	...
	ordermenu = { -- menu degli ordini
		...
		px = -0.5,
		py = CanvasY - 415 - 145,
--		ancora_x = 205,			-- posizione globale da sinistra
		ancora_x = x_orderMenu,
		ancora_y = 165,			-- posizione globale dal fondo		
		isx = iconsdimensions,
		isy = iconsdimensions,
		ix = x_Nicon_order,		-- n° icone per riga
		iy = y_Nicon_order, 	-- n° icone per colonna
...
--		color2 = {0,0,0,1}, -- riquadro interno che ho rimosso (vedi codice successivo)
		...
		cbackground = {0,0,0,0}, -- {0.03,0.18,0.3,0.5} lo nascondo per lasciar spazio all'immagine -- molix
		cborder = {0,0,0,0},  -- bordo del riquadro esterno era azzurro {0,0.67,0.99,1} lo nascondo per lasciar spazio all'immagine -- molix
		background_texture = ":n:"..LUAUI_DIRNAME.."images/menu/buildmenu/order_bkgnd.png", -- ############## todo ################ impostare small, medium, big ecc le varie taglie dimensionali. molix
...
local function AutoResizeObjects() --autoresize v2
	...
	if ((lx ~= vsx) or (ly ~= vsy)) then
		...
		for i=1,#objects do
			...
			if (o.px) then o.px = o.px * scale end						-- rescaling della posizione x
			if (o.py) then o.py = o.py * scale end						-- rescaling della posizione y
--			if (o.sx) then o.sx = o.sx * scale end						-- rescaling della dimensione x -> elimino, molix
--			if (o.sy) then o.sy = o.sy * scale end						-- rescaling della dimensione y -> elimino, molix
--			if (o.fontsize) then o.fontsize = o.fontsize * scale end	-- rescaling del testo
			if (adjust > 0) then
				o._moveduetoresize = true
				o.px = o.px - adjust
...
local function AutoResizeObjects() -- autoresize v3 (Fixed Spacing)
	if (LastAutoResizeX==nil) then
		LastAutoResizeX = CanvasX
		LastAutoResizeY = CanvasY
	end
	...
	if ((lx ~= vsx) or (ly ~= vsy)) then
		...
		-- 1. Identifichiamo quali oggetti sono "Slaves" (figli) di altri
		-- In questo widget, le icone sono slaves del background.
		local isSlave = {}
		for i=1,#objects do
			local o = objects[i]
			if (o.movableslaves) then
				for j=1,#o.movableslaves do
					isSlave[o.movableslaves[j]] = true
				end
			end
		end

		-- 2. Processiamo gli oggetti
		for i=1,#objects do
			local o = objects[i]
			
			-- Processiamo solo i "Master" (i contenitori che non sono slave di nessuno)
			if not isSlave[o] then
				local oldPx, oldPy = o.px, o.py
				
				-- Scaliamo la posizione del contenitore principale
--				if (o.px) then o.px = o.px * scale end -- originale
--				if (o.py) then o.py = o.py * scale end -- originale
--				if (o.py) then o.py = vsy-150 end  -- funzionante ma per tutti i menu (inserire if then ecc)
               if (o.ancora_x) then 
                    o.px = math.floor(o.ancora_x + 0.5) --o.px = o.ancora_x  -- Fissato a sinistra (es. 10px)
                end
                
                if (o.ancora_y) then 
                    o.py = math.floor(vsy - o.ancora_y + 0.5) --o.py = vsy - o.ancora_y -- Fissato dal fondo (es. vsy - 150)
                end

				
				-- NON scaliamo o.sx e o.sy (dimensioni fisse richieste)
				
				-- 3. Se il Master si è spostato, trasciniamo i figli mantenendo l'offset fisso
				if (o.movableslaves) then
					local dx = (o.px or 0) - (oldPx or 0)
					local dy = (o.py or 0) - (oldPy or 0)
					
					for j=1,#o.movableslaves do
						local s = o.movableslaves[j]
						if (s.px) then s.px = math.floor(s.px + dx + 0.5) end --if (s.px) then s.px = s.px + dx end
						if (s.py) then s.py = math.floor(s.py + dy + 0.5) end --if (s.py) then s.py = s.py + dy end
						-- Dimensioni e font degli slave restano invariati (pixel fissi)
					end
				end
			end
		end

		-- 4. Logica di "Adjust" (per non far finire il menu fuori dallo schermo)
		for i=1,#objects do
			local o = objects[i]
			if not isSlave[o] and o.sx then
				local adjust = (o.px + o.sx) - vsx
				if (adjust > 0) then
					o.px = o.px - adjust
					if (o.movableslaves) then
						for j=1,#o.movableslaves do
							o.movableslaves[j].px = o.movableslaves[j].px - adjust
						end
					end
				end
			end
...
--			background2.sy = self.sy - self.padding - self.padding
	...
	local selecthighlight = {"rectanglerounded",
		...
		border={0.8,0,0,0},
		glone=0.12,
		texture = LUAUI_DIRNAME.."Images/menu/buildmenu/button-pushed.png", -- bottone premuto???
		texturecolor={1,1,1,1},
	...
	mouseoverhighlight.texture = LUAUI_DIRNAME.."Images/menu/buildmenu/button-hover.png"
	...
	heldhighlight.texture = LUAUI_DIRNAME.."Images/menu/buildmenu/button-pushed_test.png"
...
--	New(background2)
	...
	backward.texture = LUAUI_DIRNAME.."Images/menu/buildmenu/backward.png"

	local forward = New(Copy(icon,true))
	forward.texture = LUAUI_DIRNAME.."Images/menu/buildmenu/forward.png"
