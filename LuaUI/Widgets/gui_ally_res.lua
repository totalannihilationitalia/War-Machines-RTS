...
-- rev1 = 	Fixed bar positions. 06/07/2026 Molix

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

local TOOL_TIPS = true
-- IMPOSTAZIONI DI POSIZIONE FISSA (in pixel)
local OFFSET_X = 15  -- Distanza dal bordo sinistro dello schermo
local OFFSET_Y = 170  -- Distanza dal bordo inferiore dello schermo
...
local function updateStatics()
  ...
  staticList = gl.CreateList( function()
    ...
    gl.Color(0.03, 0.18, 0.3, 0.45)
    gl.Rect(x1, y1, x1+w,y1+h)
--	test rettangolo
--	 gl.Color(1, 1, 1, 1) -- Mantieni 1, 1, 1, 1 se vuoi l'immagine originale, oppure riduci l'ultimo valore (es. 0.8) per la trasparenza
--   gl.Texture(background_texture) -- Sostituisci con il percorso della tua immagine di sfondo
--   gl.TexRect(x1, y1, x1+w, y1+h)
--   gl.Texture(false) -- Rilascia la texture
	
    gl.Color(0.04, 0.68, 0.98, 1)
...
--[[ rev0 rimosso
    ...
    -- Mantengo fisso l'ancoraggio in basso a sinistra (y1 non deve traslare)
    y1 = OFFSET_Y
    prevHeight = nil	
    updateStatics()
  end
  ...
  displayList = gl.CreateList( function()
    ...
    for _,d in pairs(teamRes) do
      if d.eRec then
        gl.Color(0.8, 0, 0, 0.8)		-- sfondo barra: rosso se stai inviando energia a quell'alleato
      else
        gl.Color(0.8, 0.8, 0, 0.3)		-- sfondo barra: Colore giallo scuro/semi-trasparente di base
      end
      gl.Rect(d.ex1,d.ey1,d.ex2,d.ey2)
      gl.Color(0.33,0.45,0.81,1)		-- colore barra energia - grigio/azzurro
      gl.Rect(d.ex1,d.ey1,d.ex2b,d.ey2) 
      if d.mRec then
        gl.Color(0.8, 0, 0, 0.8)		-- sfondo barra: Colore rosso se stai inviando metallo a quell'alleato
      else
        gl.Color(0.8, 0.8, 0.8, 0.3)	-- sfondo barra: Colore grigio semi-trasparente di base
      end
      gl.Rect(d.mx1,d.my1,d.mx2,d.my2)
      gl.Color(1,0.8,0,1)				-- colore barra metallo - arancio
...
labelText[1] = 
			    {
				  label=defs.name,
				  x=x1 + w + 10, -- Spostato sul lato destro (x1 + larghezza widget + 10 pixel di margine)
				  y=defs.iy1-BAR_SPACER,
				  size=TOTAL_BAR_HEIGHT,
				  config="on",  -- Cambiato da "orn" a "on" per allinearlo a sinistra
			    }
			    labelText[2] = 
			    {
				  label="(E: +"..sF("%.1f",eInc+eRec) ..", M: +"..sF("%.2f",mInc+mRec)..")", 
				  x=x1 + w + 10, -- Spostato sul lato destro
				  y=defs.iy1-BAR_SPACER-TOTAL_BAR_HEIGHT, 
				  size=TOTAL_BAR_HEIGHT/1.25, 
				  config="on",  -- Cambiato da "orn" a "on" per allinearlo a sinistra
			    }				
...
--[[  
    ...
    -- Logica "moving" rimossa per disattivare il trascinamento manuale
...
function widget:MousePress(x, y, button)
  ...
  --[[ rev0 - Rimosso il blocco che consentiva il movimento tramite clic centrale o intestazione
...
--  return {x1=x1, y1=y1, h=h}
	return nil -- Ritorna nil per ignorare il salvataggio della posizione nei file di configurazione locali
end

function widget:SetConfigData(data) --load config
--[[  rev0 -- Disabilitato per evitare che posizioni precedentemente salvate sovrascrivano i valori fissi
