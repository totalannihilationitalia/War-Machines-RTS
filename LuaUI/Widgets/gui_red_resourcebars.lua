...
-- 08/04/2026 -- rev0 modificato da molix, eliminato il fattore "scaling" per rendere più nitide le immagini. Utilizzati background images. 
-- 01/07/2026 -- rev1 Aggiungo la barra del livrium SE la fazione del giocatore è EUF o AND
...
local Config = {
	livrium = {
			...
			cbar = {0.6, 0.1, 0.8, 1}, 	-- Colore viola/violaceo per il Livrium
			...
			cicona = ":n:"..LUAUI_DIRNAME.."Images/menu/resourcesmenu/livrium_icon.png",	
			dragbutton = {2},
			tooltip = {
...
local livrium = nil -- rev1 inizializzo come nil questa variabile, servirà ad attivare la barra del livrium (ad esempio se la fazione del giocatore è EUF o AND)
...
local function updatebar_livrium(b)
	local teamID = sGetMyTeamID()
	
	-- Leggiamo i valori aggiornati dal nostro gadget del Livrium
	local cur = Spring.GetTeamRulesParam(teamID, "livrium_current") or 0
	local cap = Spring.GetTeamRulesParam(teamID, "livrium_max") or 0
	
	local barbackpx = b.barbackground.px
	local barbacksx = b.barbackground.sx
	
	-- Calcolo della percentuale di riempimento della barra
	if cap > 0 then
		b.bar.sx = (cur / cap) * barbacksx
	else
		b.bar.sx = 0
	end
	
	if (b.bar.sx > barbacksx) then
		b.bar.sx = barbacksx
	end
	
	-- Al momento non tracciamo entrate/uscite (income/pull) per il Livrium, quindi le mostriamo a 0
	b.income.caption = "+ "..short(0)
	b.pull.caption = "- "..short(0)
	b.current.caption = short(cur)
	b.storage.caption = short(cap)
	
	-- Allineamento dei testi (copiato dalla logica originale del widget)
	...
	-- Disattiviamo l'indicatore di condivisione risorsa (visto che il Livrium non è condivisibile nativamente)
	b.shareindicator.active = false
...
function widget:Initialize()
	...
	-- aggiungere livrium?? per ora non faccio in modo che sia condivisibile
