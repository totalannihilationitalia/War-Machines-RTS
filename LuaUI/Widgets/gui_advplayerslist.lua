...
local pics = {
	...
	unitsPic        = imageDirectory.."unita.dds",
	energyPic       = imageDirectory.."energia.dds",
--	metalPic        = imageDirectory.."metal.png",
	metalPic        = imageDirectory.."metallo.dds",
...
--To determine faction at start ********************************************** sostituisco armcom con icucom e nfacom e aggiungo i due comandanti di Kickstarter
local icucomDefID = UnitDefNames.icucom.id
...
function SetSidePics() 
	...
	--set factions, from TeamRulesParam when possible and from initial info if not *************************************************************** sostituisco armcom con icucom ecc ecc ************
	teamList = Spring_GetTeamList()
	for _, team in ipairs(teamList) do
		local teamside
		if Spring_GetTeamRulesParam(team, 'startUnit') then
			local startunit = Spring_GetTeamRulesParam(team, 'startUnit')
--************************************************************************************************* modifico le condizioni per inserire i comandanti KICu e KNFA
			if startunit == icucomDefID  then 
				teamside = "icu"
...
-- *********************************************************************************************questo affare è quello che fà uscire il punto di domanda...ma che è????????????????
...
-- ******************************* qui sotto la stringa del punto di domanda
...
function DrawShareButtons(posY, needm, neede) -- funzioni share unit, share metal, share energy con convo box per avvertire
	gl_Color(1,1,1,1)
	...
	if needm == true then  -- se c'è bisogno di metallo disegna la freccia lowpic sull'icona del metallo
		DrawRect(m_share.posX + widgetPosX  + 33, posY, m_share.posX + widgetPosX  + 49, posY + 16) -- disegna lowpic sull'icona del metallo
	end
	if neede == true then
		DrawRect(m_share.posX + widgetPosX  + 17, posY, m_share.posX + widgetPosX  + 33, posY + 16)	
	end
