...
--
--  file:    WMRTS_menu.lua
--  brief:   War Machines RTS options menu
...
--------------------------------------------------------------------------------
-- rev 0 by molix
-- at the moment, only menu sound is present
...
local larghezza_barre =  200 		-- larghezza barre per impostazioni a scorrimento (es volume, particelle, shadow quality etc)
local altezza_barre = 10			-- altezza barre  per impostazioni a scorrimento (es volume, particelle, shadow quality etc)
local altezza_menu_snd = 140		-- altezza della finestra menu sound
local larghezza_menu_snd = 400		-- larghezza della finestra menu sound
...
local posx_menu = vsx/2 - larghezza_menu_snd/2 	-- parte da sx
local posy_menu = vsy/2 - altezza_menu_snd/2 	-- parte dal basso
...
local function UpdateGeometry() -- aggiorno geometria
  posx_menu = vsx/2 - larghezza_menu_snd/2
  posy_menu = vsy/2 - altezza_menu_snd/2
end
--UpdateGeometry()
...
--  local xvol = xmin + volume_master * (xmax - xmin) -- sostituito da:
  ...
  gl.Color(.3, .3, .3)
	...
	gl.TexRect(	posx_menu,posy_menu,posx_menu+larghezza_menu_snd,posy_menu+altezza_menu_snd)	
...
-- gui shader	
		...
		if (WG['guishader_api'] ~= nil) then
		WG['guishader_api'].InsertRect( posx_menu,posy_menu, posx_menu+larghezza_menu_snd, posy_menu+altezza_menu_snd,'WMRTS_snd_option')
	end
