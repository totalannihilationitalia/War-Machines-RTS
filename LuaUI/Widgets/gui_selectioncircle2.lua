...
local circleDivs   = 8 -- 25precisione del cerchio (numero di lati)
...
-- sostituisco le stringhe originali sopra per aggiungere un effetto sfumato al cerchio delle unità selezionate		
--						local a1 = (i * radstep)
--						local a2 = ((i+1) * radstep)
--						glColor( 1, 0, 0, 0 )  -- sostituisco il colore fisso con il colore della squadra ---- glColor( 1, 0, 0, 0 )
--						gl.Vertex(math.sin(a1), circleOffset, math.cos(a1)) -- cerchio interno x, y
--						gl.Vertex(math.sin(a2), circleOffset, math.cos(a2)) --
--						glColor( 1, 0, 0, 0.4 ) -- sostituisco il colore fisso con il colore della squadra ---- glColor( 1, 0, 0, 0.4 )
--						gl.Vertex(math.sin(a2)*1.1, circleOffset, math.cos(a2)*1.2)
--						gl.Vertex(math.sin(a1)*1.2, circleOffset, math.cos(a1)*1.2) -- dovrebbero essere parametri x,y,z
--
--
      end
    end)
...
local function SetUnitColor(unitID)
  ...
  if (teamID == nil) then
    glColor(0.0, 1.0, 1.0, 0.75) -- red 
  elseif (teamID == GetMyTeamID()) then
    glColor(1, 1.0, 1.0, 1) -- ERA cyan (0.0, 1.0, 1.0, 0.75)
	--glColor( 1, 0, 0, 0.4 )
...
---------------------------
 --- SPESSORE LINEA PER LE UNITA' SELEZIONATE
----------------------------
...
-------------------------------------------
-- cerchio per le unità selezionate che volano
-------------------------------------------
            if (UnitDefs[unitDefID or -1].canFly) then radius = radius*1.5 end -- radius era 1.7
    ...
    if (data ~= unitID) then
      ...
      if (radius) then
        ...
        if (x) then
          ...
          local degrot = acos(gy) * PI_DEG
-----------------------------------------
-- spessore lina al passaggio del mouse
-----------------------------------------
          glLineWidth(2) --3.5
