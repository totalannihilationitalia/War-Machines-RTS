...
function widget:DrawWorldPreUnit()
    ...
    for i=1, #visibleUnits do
        ...
        if extractorType and extractorType > 1.0 then		
            local x, y, z = Spring.GetUnitPosition(uID)
            DrawMetalAura(x, y, z, extractorType)
        end		
    end
    ...
    for i=1, #visibleUnits do
		...
		if rafinery then 	-- disegna lo stato della raffineria ON in base al valore "rafinery"
			...
			if not active then
				...
				local h = (UnitDefs[unitDefID] and UnitDefs[unitDefID].height or 30)
				-- Disegna Icona
				DrawRafineryIcon(x, y, z, h, rafinery) -- ######################## creare la funzione!!!!!! 
...
function widget:DrawScreen()
    -- 1. Unità Obbiettivo
    local allUnits = Spring.GetAllUnits() -- Qui controlla tutte le unità ogni frame, e non solo le visibili ############################################### creare una tabella "allUnits" da aggiornare in in widget:Update() ogni "n" secondi. attualmente il sistema aggiorna tutte le unità ogni frame, creando lag ######################
    for i=1, #allUnits do
        local uID = allUnits[i]
        local objType = Spring.GetUnitRulesParam(uID, "obj_type")
        if objType and objType > 0 then
            local x, y, z = Spring.GetUnitPosition(uID)
            DrawEdgeMarker(x, y, z, objType)
        end
    end
