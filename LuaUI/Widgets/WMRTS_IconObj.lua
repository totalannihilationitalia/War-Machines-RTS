...
-- Config immagini icone e marcatori per missioni
...
-- Config immagine indicatore laterale per missioni
local img_edge_attack = "LuaUI/Images/menu/objectives/edge_arrow_attack.png" 
local img_edge_defend = "LuaUI/Images/menu/objectives/edge_arrow_defend.png" 
local img_edge_move = "LuaUI/Images/menu/objectives/edge_arrow_move.png" 
local img_edge_build = "LuaUI/Images/menu/objectives/edge_arrow_build.png" 
local img_edge_select = "LuaUI/Images/menu/objectives/edge_arrow_select.png" 
local img_edge_construct= "LuaUI/Images/menu/objectives/edge_arrow_construct.png" 
local img_edge_danger = "LuaUI/Images/menu/objectives/edge_arrow_danger.png" 

-- Config immagini icone e marcatori per estrattori di metallo nel range dei resonator
local icon_extr_on = "LuaUI/Images/menu/objectives/icon_extron.png"
...
local MAX_FIXED_POINTS = 10 					-- Limite massimo di punti fissi
local vsx, vsy = widgetHandler:GetViewSizes()	-- cattura la dimensione dello schermo
local unitsWithIcons = {} 						-- Tabella unità con icone: [unitID] = {type, height, status, ecc}
--------------------------------------------------------------------------------

-- funzione per rilevare la dimensione dello schermo (risoluzione)
...
-- Funzione per disegnare l'aurea degli obiettivi (sotto l'unità)
local function DrawAura(x, y, z, type)
    ...
    local tex = aura_move
		...
		elseif type == 2 or type == 10 then tex = aura_defend
...
-- Funzione per disegnare l'aurea degli estrattori di metallo (sotto l'unità)
local function DrawMetalAura(x, y, z, type)
-- type sarà 2.0 o 3.0 , quando verrà chiamato questa funzione locale #################################################################################
    local time = Spring.GetGameSeconds()
    local pulse = 0.5 + math.sin(time * 2) * 0.2
    local rotation = time * 60 
    local tex = aura_extr_off
		if type == 2.0 then tex = aura_extr_off
		elseif type == 3.0 then tex = aura_extr_on
		end
    gl.DepthMask(false) -- disabilito la scrittura della profondità
    gl.DepthTest(true)
    gl.Blending(true)
...
--    local bounce = math.sin(time * 3) * 8 			-- non deve rimbalzare ma rimanere statica
    ...
    if type == 2.0 then tex = icon_extr_off
    elseif type == 3.0 then tex = icon_extr_on
	end
...
-- Funzione per disegnare l'icona (sopra le raffinerie)
local function DrawRafineryIcon(x, y, z, height, status)
--    local time = Spring.GetGameSeconds()
--    local bounce = math.sin(time * 3) * 8 			-- non deve rimbalzare ma rimanere statica
...
local function DrawEdgeMarker(wx, wy, wz, type)
    ...
    local margin = 40 -- Distanza dal bordo dello schermo
    
    -- Se sz > 1 l'oggetto è dietro la telecamera
    -- Se sx/sy sono fuori dai limiti [0, vsx] l'oggetto è fuori schermo
    if sz > 1 or sx < 0 or sx > vsx or sy < 0 or sy > vsy then
        local centerX, centerY = vsx * 0.5, vsy * 0.5
        local dx, dy = sx - centerX, sy - centerY
        
        -- Se l'oggetto è dietro, invertiamo i vettori
        if sz > 1 then
            dx, dy = -dx, -dy
        end
        
        -- Calcolo dell'angolo per la rotazione della freccia
        local angle = math.atan2(dy, dx)
        local angleDeg = math.deg(angle)
        
        -- Troviamo il punto d'intersezione col bordo dello schermo (clamping)
        local edgeX = centerX + dx
        local edgeY = centerY + dy
        
        local ratioX = (vsx * 0.5 - margin) / math.abs(dx)
        local ratioY = (vsy * 0.5 - margin) / math.abs(dy)
        ...
        elseif type == 6 then edgeTex =img_edge_construct		
        elseif type == 7 then edgeTex = img_edge_danger
        end

        -- logica del lampegggio 
        local time = Spring.GetGameSeconds()
        local blink = 0.65 + math.sin(time * 10) * 0.35 
        -- Scegliamo il colore in base al tipo (opzionale, qui bianco)
		
		
        gl.Color(1, 1, 1, blink)
        gl.Texture(edgeTex)
        
        gl.PushMatrix()
        gl.Translate(edgeX, edgeY, 0)
        gl.Rotate(angleDeg - 90, 0, 0, 1) -- Ruota verso l'obiettivo (assume freccia punti UP)
...
function widget:DrawWorldPreUnit()
    for uID, data in pairs(unitsWithIcons) do
		if Spring.ValidUnitID(uID) then		-- e l'unità è ancora viva
			-- Disegniamo l'aurea solo se l'unità è effettivamente visibile alla telecamera
			if Spring.IsUnitVisible(uID) then
				local x, y, z = Spring.GetUnitPosition(uID)
				if data.objType > 0 then
					DrawAura(x, y, z, data.objType)
				end
				if data.extType > 1.0 then
					DrawMetalAura(x, y, z, data.extType)
				end
			end
		end
    end
...
function widget:DrawWorld()
    ...
    for uID, data in pairs(unitsWithIcons) do
        if Spring.IsUnitVisible(uID) then
            ...
            if data.objType > 0 then
                DrawIcon(x, y, z, h, data.objType)
                DrawLabel(x, y, z, h, typeToText[data.objType])
            end
            
            if data.rafStat then
                local s = data.isActive and data.rafStat or 3
                DrawRafineryIcon(x, y, z, h, s)
            end

            if data.extType > 1.0 then
                DrawResonatorIcon(x, y, z, h, data.extType)
            end
...
-- 1. AGGIORNAMENTO LENTO (Ogni 15 frame / 0.5 sec)
-- Invece di GetAllUnits ogni frame, lo facciamo raramente
function widget:Update()
    ...
    for i=1, #allUnits do
        ...
        local objType = Spring.GetUnitRulesParam(uID, "obj_type") or 0
...
-- disegna l'indicatore sull' HUD
function widget:DrawScreen()
    -- 1. Unità Obbiettivo
    for uID, data in pairs(unitsWithIcons) do
        if data.objType > 0 then
            local x, y, z = Spring.GetUnitPosition(uID)
            DrawEdgeMarker(x, y, z, data.objType)
        end
    end

    -- 2. Punti Fissi Obbiettivo
    for i=1, MAX_FIXED_POINTS do
        local prefix = "obj_marker_" .. i
        if (Spring.GetGameRulesParam(prefix .. "_active") or 0) == 1 then
            local px = Spring.GetGameRulesParam(prefix .. "_x") or 0
