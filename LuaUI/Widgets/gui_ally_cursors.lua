...
function widget:GetInfo()
  return {
    name      = "AllyCursors",
    desc      = "Shows the mouse pos of allied players. Hold META-key to display player names near mouse cursors.",
    author    = "jK",
    date      = "May,2008",
...
--modified by vbs:
--hold META to print player names near mouse cursors
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

-- configs

local sendPacketEvery = 0.5 -- era 0.8
    ...
    for n=0,5 do
      ...
      if (IsSphereInView(wx,gy,wz,16)) then
        ...
        if (data[#data-1]) then --mouse pressed?
          ...
          color = SetTeamColor(teamID,n*0.2)
        end
		
		glTexture('LuaUI/Images/AlliedCursors.png') --reassign texture for drawName                      
		...
		if ( meta and playerNames[playerID] ) then
			...
			glBillboard()
			font:Begin()
			font:SetOutlineColor( {0,0,0,color[4]} )
			font:SetTextColor( color )
			font:Print( playerNames[ playerID ], 0, 0, fontSize, "cO" )	
			font:End()
			glPopMatrix()  
		end
		--end of draw player
