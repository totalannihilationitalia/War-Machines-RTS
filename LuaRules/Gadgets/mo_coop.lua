...
local COMMANDER = {
  [UnitDefNames["nfacom"].id] = true,
  [UnitDefNames["icucom"].id] = true,
}
local comStrings = {
  [1] = "nfacom",
  [2] = "icucom",
  ...
  if preStart and startPoints[id] then
    ...
    elseif (msg:find("coop_factionchange",1,true)) then
      ...
      if id == (playerID * 1) then
        if startPoints[id].commander == "icucom" then
          startPoints[id].commander = "nfacom"
        else
          startPoints[id].commander = "icucom"
        end
