...
local function Jump(unitID, finish)
  ...
  spCallCOBScript(unitID, "BeginJump", 0)  -- richiama lo script "BeginJump" definito nel cob delle unità interessate (vedi jump_defs.lua)
  spSetUnitRulesParam(unitID,"jumpReload",0) -- richiama lo script "jumpReload" definito nel cob delle unità interessate (vedi jump_defs.lua)
  ...
  if (jumpDef.aaShootMe) then
    fakeUnitID = spCreateUnit("icupatroller", start[1], start[2], start[3], "n", teamID)
    mcEnable(fakeUnitID)
