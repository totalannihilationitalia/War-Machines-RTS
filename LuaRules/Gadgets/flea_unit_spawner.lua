...
if (gadgetHandler:IsSyncedCode()) then
	...
	do -- load config file
	  local CONFIG_FILE = "LuaRules/Configs/flea_spawn_defs.lua"
	...
	local function SpawnBurrow()
	  ...
	  repeat
		...
		tries = tries + 1
		local blocking = Spring.TestBuildOrder(UnitDefNames["icumetex"].id, x, y, z, 1)
		if (blocking == 2) then
	...
	local function SpawnQueen()
	 ...
	 repeat
		...
		y = Spring.GetGroundHeight(x, z)
		tries = tries + 1
		local blocking = Spring.TestBuildOrder(UnitDefNames["icupatroller"].id, x, y, z, 1)
