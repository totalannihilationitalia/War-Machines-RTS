...
if (gadgetHandler:IsSyncedCode()) then
   ...
   function GeneratePatrolPoints(teamID, data)
      ...
      if #enemyTeamsFound > 0 then
          ...
          for i, enemyTeamID in ipairs(enemyTeamsFound) do
              ...
              if ex and ez then
                  -- Aggiungiamo un punto a metà strada verso il nemico
                  ...
                  -- Potresti anche aggiungere un punto vicino alla base nemica
                  -- local enemyBasePatrolX = ex + math.random(-150, 150)
                  -- local enemyBasePatrolZ = ez + math.random(-150, 150)
                  -- table.insert(tempPoints, { x = enemyBasePatrolX, z = enemyBasePatrolZ, type = "ENEMY_BASE_VICINITY_" .. enemyTeamID })
              else
                  Log(teamID, "GeneratePatrolPoints: Could not get start position for enemy team " .. enemyTeamID)
              end
              if i >= 2 then -- Limitiamo per non avere troppi punti iniziali, puoi aggiustare
                  Log(teamID, "GeneratePatrolPoints: Limiting enemy approach points generation to first "..i.." enemies for now.")
                  break
              end
          end
      else
  ...
  -- Funzione per creare un nuovo gruppo di combattimento (già nel tuo codice, assicurati sia questa versione)
  local function CreateNewCombatGroup(teamID, data, unitIDs, frame)
      local newGroupID = data.nextGroupID; data.nextGroupID = data.nextGroupID + 1
      local newGroup = {id = newGroupID, units = {}, state = "idle", targetPoint = nil, currentPatrolIndex = nil, lastOrderFrame = frame }
      for _,uID in ipairs(unitIDs)do if data.combatUnits[uID]then table.insert(newGroup.units,uID);data.combatUnits[uID].state="assigned_to_group";data.combatUnits[uID].groupID=newGroupID else Log(teamID, "CreateNewCombatGroup: Warning - Unit " .. uID .. " not found in combatUnits.") end end
      if #newGroup.units > 0 then data.combatGroups[newGroupID] = newGroup; Log(teamID,"Created new combat group "..newGroupID.." with "..#newGroup.units.." units.")
      else Log(teamID,"CreateNewCombatGroup: Attempted to create an empty group (ID "..newGroupID.."), skipping."); data.nextGroupID = data.nextGroupID - 1 end
  end
